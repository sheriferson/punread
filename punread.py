"""
A Python script and bitbar plugin to show count of unread pinboard links.


punread.py calls my pinboard account, gets number of unread bookmarks, and a random sample
of the unread links for access from the menu bar via bitbar.
"""
...
def print_random_unread_links(count, unread, nlinks=30):
    """
    Accepts the count of all unread bookmarks (count), a list of the bookmarks (unread)
    and the number of random links to pull out of the list and display in the menu (nlinks).
    With those inputs, this function prints output formatted for [Bitbar] to stdout.
    [Bitbar]: https://github.com/matryer/bitbar
    """

    count = str(count) + ' | font=SourceSansPro-Regular color=dodgerblue\n---\n'
    sys.stdout.buffer.write(count.encode('utf-8'))
    random_unread_indexes = random.sample(range(1, len(unread)), nlinks)
    for ii in random_unread_indexes:
        description = unread[ii]['description']
        description = description.replace("|", "｜")
        link_entry = description +   \
                " | href=" +         \
                unread[ii]['href'] + \
                " font=SourceSansPro-Regular color=dodgerblue\n"
...
if __name__ == '__main__':
    # get the number of random links to show if provided
    try:
        nlinks = int(sys.argv[1])
    except:
        nlinks = 10
    ...
    if os.path.isfile(last_run_path):
        ...
        if time.time() - last_run < 300:
            ...
            print_random_unread_links(unread_count, unread, nlinks)
            exit()
        else:
